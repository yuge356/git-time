...
- 未登录用户访问 `/` 时先看到 DayFlow 欢迎页；首张轮播使用网页原生横向任务树动效，按照“叶子任务完成 → 连接线回传 → 父任务完成 → 项目根节点完成”的顺序演示时间预算和项目进度，节点始终使用圆角矩形并提供克制的液态填充、路径式对号与最终轻微光泽；后续依次展示最新的今日任务、时间统计和项目任务树真实截图，并提供清晰的登录、注册入口，手机端使用触摸滚动与单列布局
