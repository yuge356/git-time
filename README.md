...
- “项目 / 课程 → 模块 → 可执行任务”三级任务树，可在任务树与微玻璃拟态大纲列表视图间切换，具备稳定项目色彩识别系统；大纲列表中的父任务默认折叠、使用下拉箭头展开，以留白缩进代替左侧连接线，同级子任务整齐对齐且每个节点只显示一条进度轨道，导图按内容撑开并随整个网页滚动查看
- 项目、模块和任务统一使用弹窗创建/编辑；新建可执行任务必须设置大于 0 的计划用时，并支持折叠、拖拽层级、进度、优先级、截止日期与任务依赖
- 开始、暂停、恢复和结束学习计时；达到计划用时时显示站内/浏览器提醒但继续计时；关闭网页自动暂停，跨越午夜也会在当天 00:00 暂停并切换到新一日；项目和模块汇总末级任务的预算与实际时长
- 每日计划、长期任务引用、临时事项、完成进度与连续打卡；项目任务的计划用时修改后同步到今日任务和正在运行的计时目标。今日任务返回页面时优先显示本地缓存，新增事项立即显示并在后台同步
...
- 内容分类卡片使用统一的纯色背景，不使用渐变底色
- 欢迎页、认证卡片、首次使用指引、应用侧栏与主要数据卡片采用克制的轻度玻璃层级：主要数据卡保持接近实色且不使用实时背景模糊，模糊仅保留在侧栏和小型浮层，避免影响页面切换；正文、表单与图表保持稳定清晰
