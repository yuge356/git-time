...
- 每日计划、长期任务引用、临时事项、完成进度与连续打卡；今日任务勾选完成状态即时同步到项目任务树（以大勾选符号显式标示）；项目页新建或修改任务（截止日期为今天或重复规则匹配今天）后切换回今日页会自动导入，无需手动添加；今日页面左侧为专注计时、右侧为今日任务列表，列表卡片高度固定在空状态时的尺寸，不随任务数量增长，超出容量的任务在列表内部垂直滚动查看；专注计时和今日任务下方为计划进度表（甘特图：天 / 周 / 月三档时间轴且选择保存在本地，今日紫色指示线，周末浅色底纹；任务按项目分组可折叠，条形按真实学习日期分段绘制、中断显示间隔并以颜色区分投入进度；顶部展示总学习时长、活跃任务数、完成率与连续学习天数；每行可叠加虚线计划窗口，支持拖拽平移 / 缩放排期或在行内空白处拖出新窗口，保存到任务计划起止日期；悬浮显示任务详情，点击跳转项目页任务详情；支持拖拽平移与横向滚动，手机端自动切换为卡片视图），再下方并排展示月度计时日历与按小时统计的今日专注分布（仅展示选定单日数据；右上角提供日期切换，可回看历史某一天的柱状图，未来日期不可选；悬浮柱形弹窗显示该时刻与该小时累计学习时长），窄屏自上而下依次排列并提供通透宽松的排版
