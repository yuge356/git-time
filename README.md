...
- 日期范围统计、每日趋势平滑折线图（白底紫色线条，线下方为上深下透明的半透明紫色渐变填充；默认不显示顶点圆点与数值，悬停顶点弹出白色数据提示框；日期刻度格式为"月.日"，默认展示最近 10 天）、任务投入分布与预算偏差；切换到统计页时先即时渲染上次结果再后台刷新，本地计时同步并行进行、仅在确有离线变更时补一次刷新
