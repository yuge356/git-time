...
- “项目 / 课程 → 模块 / 任务 → 可执行任务（可再含一层子任务）”灵活任务树，整体采用 Apple Liquid Glass 风格结合 2.5D 扁平化设计；背景使用柔和的冷灰与冰蓝渐变环境光晕，中央毛玻璃卡片（严格圆角矩形）具备高对比度清晰边框与轻盈投影；节点间使用连续无断点的 2px 连接线（父级短线、垂直主干与子级短线精确对接）；项目下可直接添加任务或模块，任务下可再添加一层子任务；可在大纲列表与任务树间平滑切换，任务树模式下每个项目一块独立画布，标题旁可折叠/展开该项目任务树，节点保持固定大小，调整窗口大小时内容不缩放、仅由画布裁剪滚动
...
- 每日计划、长期任务引用、临时事项、完成进度与连续打卡；今日任务勾选完成状态即时同步到项目任务树（以大勾选符号显式标示）；今日页面左侧为专注计时、右侧为今日任务列表，列表卡片高度固定在空状态时的尺寸，不随任务数量增长，超出容量的任务在列表内部垂直滚动查看，底部并排展示月度计时日历与按小时统计的今日专注分布（仅展示选定单日数据；右上角提供日期切换，可回看历史某一天的柱状图，未来日期不可选；悬浮柱形弹窗显示该时刻与该小时累计学习时长），最下方为只读计划进度表（横轴为"月.日"日期、纵轴为有学习记录的任务，条形表示学习时间跨度，悬浮显示累计时长与每日明细，支持横向滚动查看更多日期），窄屏自上而下依次排列并提供通透宽松的排版
