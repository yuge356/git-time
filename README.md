...
- “项目 / 课程 → 模块 / 任务 → 可执行任务（可再含一层子任务）”灵活任务树，整体采用 Apple Liquid Glass 风格结合 2.5D 扁平化设计；背景使用柔和的冷灰与冰蓝渐变环境光晕，中央毛玻璃卡片（严格圆角矩形）具备高对比度清晰边框与轻盈投影；节点间使用连续无断点的 2px 连接线（父级短线、垂直主干与子级短线精确对接）；项目下可直接添加任务或模块，任务下可再添加一层子任务；可在大纲列表与任务树间平滑切换，任务树模式下每个项目一块独立画布，节点保持固定大小，调整窗口大小时内容不缩放、仅由画布裁剪滚动
...
- 日期范围统计、每日趋势折线图（白底紫色折线，默认展示最近 10 天）、任务投入分布与预算偏差；统计页先立即显示已有数字，本地计时同步在后台并行进行并在完成后自动补一次刷新
