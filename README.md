...
- “项目 / 课程 → 模块 → 可执行任务”三级任务树，整体采用 Apple Liquid Glass 风格结合 2.5D 扁平化设计；背景使用冷白、冰蓝、浅青与淡紫多层自然过渡渐变与环境光晕；任务节点采用半透明 Liquid Glass 材质（严格圆角矩形）配合细腻玻璃折射、柔和高光与内阴影；节点间使用纤细流畅的轻微玻璃质感连接线；可在大纲列表与任务树间平滑切换，导图随网页滚动自然呈现
...
- 每日计划、长期任务引用、临时事项、完成进度与连续打卡；今日任务勾选完成状态即时同步到项目任务树（以大勾选符号显式标示）；移动端打开时自顶向下依次排列专注计时、今日任务列表与计时日历，并提供通透宽松的日历排版
